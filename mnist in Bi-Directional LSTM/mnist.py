...
def train(model, sess, training_iters, display_step):
    train_writer = tf.summary.FileWriter('./mnist in Bi-Directional LSTM/train', sess.graph)
    sess.run(init)
    step = 0
...
def save(sess):
    ...
    save_path = saver.save(sess, "./mnist in Bi-Directional LSTM/model/model.ckpt")
...
if __name__ == "__main__":
    training_iters = 1000
    display_step = 10
    my_network = model.Bd_LSTM_layer(name="mnist")
